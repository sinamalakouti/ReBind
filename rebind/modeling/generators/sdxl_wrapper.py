...
class SDXLWrapper(T2IGenerator):
    ...
    def generate_images(
        ...
        try:
            ...
            output = self.pipe(
                prompt=prompts[i],
                negative_prompt=self.snegative_prompt,
            ...
            generated_images = [image for image in output.images]
    ...
    def forward(
        self, prompts: List[str], num_images: int = 1, size: str = "512x512"
    ) -> List[Image.Image]:
        """
        Generate images using SDXL (implements T2IGenerator interface)
        """
        return self.generate_images(prompts=prompts, num_images=num_images, size=size)
