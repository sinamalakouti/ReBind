...
def log_validation(
    ...
    test_num_validation_images = 10
    if (
        (epoch + 1) % (args.validation_epochs * 4) == 0 and epoch > 1
    ) or is_final_validation:
        num_validation_images = test_num_validation_images
        validation_triplets = [
            triplet
            for target_triplet in args.target_triplets
    ...
    for validation_triplet in validation_triplets:
        ...
        if num_validation_images == test_num_validation_images:
...
VALIDATION_TRIPLETS = {
    ...
    "horse_riding_astronaut": [
        "horse_riding_bear",
        "horse_riding_astronaut",
        "bear_riding_astronaut",
        "astronaut_riding_horse",
    ],
}
...
TRAIN_TRIPLETS = {
    ...
    "horse_riding_astronaut": [
        "horse_riding_motorcycle",
        # "horse_riding_horse",
        # "horse_riding_zebra",
        # "zebra_riding_horse",
        # "astronaut_riding_astronaut",
        # "bear_riding_astronaut",
        # "kid_riding_astronaut",
        # "cat_riding_astronaut",
        # # "astronaut_riding_cat",
        # "dog_riding_astronaut",
    ],
}

TRIPLET_ACTIVE_PASIVE = {
    "mouse_chasing_cat": {
        "active": "mouse_chasing_mouse",
        "passive": "cat_chasing_cat",
    },
    "horse_riding_astronaut": {
        "active": "horse_riding_bear",
        "passive": "bear_riding_astronaut",
    },
}

# TARGET_TRIPLET = "horse_riding_astronaut"  # "mouse_chasing_cat"
...
def create_intermediate_dataset(data_dir: str, intermediate_triplets: List[str]):
    """Create separate HuggingFace datasets for active and passive examples"""
    # Separate dictionaries for active and passive data
    active_dict = {"image": [], "caption": [], "expanded_caption": []}
    passive_dict = {"image": [], "caption": [], "expanded_caption": []}

    for target_triplet in args.target_triplets:
        _, relation, _ = parse_action_triplet(target_triplet)
        data_dir = Path(data_dir) / relation

        # Get active and passive triplets
        active_triplet = TRIPLET_ACTIVE_PASIVE[target_triplet]["active"]
        passive_triplet = TRIPLET_ACTIVE_PASIVE[target_triplet]["passive"]

        # Get directories and CSV paths for both active and passive
        active_dir = data_dir / active_triplet / "expanded_images"
        passive_dir = data_dir / passive_triplet / "expanded_images"
        active_csv = data_dir / active_triplet / "expanded_prompt_mapping.csv"
        passive_csv = data_dir / passive_triplet / "expanded_prompt_mapping.csv"

        if active_dir.exists() and passive_dir.exists():
            # Process active images
            for img_path in active_dir.glob("*.png"):

                active_dict["image"].append(Image.open(img_path).convert("RGB"))
                active_dict["caption"].append(
                    get_base_prompt_from_action_triplet(active_triplet)
                )

                expanded_prompt = get_expanded_prompt(active_csv, img_path.name)
                active_dict["expanded_caption"].append(expanded_prompt or "")

                if not expanded_prompt:
                    logger.warning(
                        f"Warning: No expanded prompt found for active image {img_path.name}"
                    )

            # Process passive images
            for img_path in passive_dir.glob("*.png"):
                passive_dict["image"].append(Image.open(img_path).convert("RGB"))
                passive_dict["caption"].append(
                    get_base_prompt_from_action_triplet(passive_triplet)
                )

                expanded_prompt = get_expanded_prompt(passive_csv, img_path.name)
                passive_dict["expanded_caption"].append(expanded_prompt or "")

                if not expanded_prompt:
                    logger.warning(
                        f"Warning: No expanded prompt found for passive image {img_path.name}"
                    )

    # Create separate HuggingFace datasets
    active_dataset = datasets.Dataset.from_dict(active_dict)
    passive_dataset = datasets.Dataset.from_dict(passive_dict)

    return active_dataset, passive_dataset
...
def main(args):
    ...
    active_dataset, passive_dataset = create_intermediate_dataset(
        data_dir=args.data_dir, intermediate_triplets=TRAIN_TRIPLETS
    )
    ...
    num_update_steps_per_epoch = math.ceil(
        min(len(active_dataloader), len(passive_dataloader))
        / args.gradient_accumulation_steps
    )
    ...
    if args.train_text_encoder:
        ...
        unet, optimizer, active_dataloader, passive_dataloader, lr_scheduler = (
            accelerator.prepare(
                unet, optimizer, active_dataloader, passive_dataloader, lr_scheduler
            )
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        train_loss = 0.0
        for step, (active_batch, passive_batch) in enumerate(
            zip(active_dataloader, passive_dataloader)
        ):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.snr_gamma is None:
                    ...
                    # Take final mean
                    loss = loss.mean() * mse_loss_weights
        ...
        if accelerator.is_main_process:
            if epoch % args.validation_epochs == 0 or step in [200, 400]:
                # print("************* VALIDATION *************")
                # # create pipeline
