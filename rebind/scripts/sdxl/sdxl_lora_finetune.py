...
def log_validation(
    ...
    test_num_validation_images = 1

    num_validation_images = (
        test_num_validation_images
        if (epoch + 1) % 5 == 0 and epoch > 1
        else args.num_validation_images
    )
...
TARGET_TRIPLETS = ["mouse_chasing_cat"]
...
def create_intermediate_dataset(data_dir: str, intermediate_triplets: List[str]):
    """Create a HuggingFace dataset from intermediate triplets"""
    data_dict = {"image": [], "caption": [], "expanded_caption": []}
    for target_triplet in TARGET_TRIPLETS:
        _, relation, _ = parse_action_triplet(target_triplet)
        data_dir = Path(data_dir) / relation

        # Collect all image-text pairs
        for triplet in intermediate_triplets[target_triplet]:
            triplet_dir = data_dir / triplet
            image_dir = triplet_dir / "expanded_images2"
            csv_path = triplet_dir / "expanded_prompt_mapping.csv"

            if image_dir.exists():
                for img_path in image_dir.glob("*.png"):
                    # Extract image number from path (assuming format image_X.png)
                    # Gets "image_X.png"
                    data_dict["image"].append(Image.open(img_path).convert("RGB"))
                    data_dict["caption"].append(
                        get_base_prompt_from_action_triplet(triplet)
                    )
                    image_name = img_path.name
                    # Get expanded prompt from CSV
                    expanded_prompt = get_expanded_prompt(csv_path, image_name)
                    if expanded_prompt is not None:
                        data_dict["expanded_caption"].append(expanded_prompt)
                    else:
                        data_dict["expanded_caption"].append("")
                        logger.warning(
                            f"Warning: No expanded prompt found for {image_name}, image_name: {image_name}"
                        )
            #   printpri(
            #         f"\nProcessing image:"
            #         f"\n  Path: {img_path}"
            #         f"\n  Name: {image_name}"
            #         f"\n  Caption: {data_dict['caption'][-1]}"
            #         f"\n  Expanded: {data_dict['expanded_caption'][-1]}"
            #     )
...
def main(args):
    ...
    dataset = create_intermediate_dataset(
    ...
    if accelerator.is_main_process:
        ...
        if args.num_validation_images > 0:
            log_validation(pipeline, args, accelerator, epoch, is_final_validation=True)
